use embedded_crc_macros::crc8_lookup_table;

crc8_lookup_table!(fn smbus_pec, 0, "SMBus Packet Error Code");

const ADDRESS: u8 = 0x5A;
const REGISTER: u8 = 0x06;

fn main() {
    let pec_write = smbus_pec(&[ADDRESS << 1, REGISTER, 0xAB, 0xCD]);
    println!("PEC: {}", pec_write); // prints 95
}

// This lookup table can be generated on build time by calling the
// `build_rs_lookup_table_file_generation` macro in a `build.rs` file in your project.
const LOOKUP_TABLE: [u8; 256] = [
    ,
];
